...
import { addToSyncQueue } from "../store/slices/offlineDataSlice";
  ...
  async (error) => {
    ...
    if (error.response.status === 401) {
      ...
      if (state.auth.refreshToken) {
        try {
          // Instead of dispatching refreshToken, we'll handle it directly here
          const tokens = await refreshUserToken(state.auth.refreshToken);
...
const apiService = {
  ...
  register: async (userData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot register while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
  ...
  login: async (credentials) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot login while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
  ...
  verifyPhone: async (data) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot verify phone while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
  ...
  requestPasswordReset: async (phoneNumber) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot request password reset while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
  ...
  uploadProfileImage: async (imageFile) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot upload image while offline. Your image will be uploaded when you are back online.",
        );
      }
      throw error;
    }
  ...
  submitVerification: async (documentData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot submit verification documents while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
  ...
  createListing: async (listingData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Your listing will be created when you are back online.",
        );
      }
      throw error;
    }
  ...
  updateListing: async (id, listingData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Your listing will be updated when you are back online.",
        );
      }
      throw error;
    }
  ...
  deleteListing: async (id) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Your listing will be deleted when you are back online.",
        );
      }
      throw error;
    }
  ...
  createTransaction: async (transactionData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Your transaction will be created when you are back online.",
        );
      }
      throw error;
    }
    ...
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        ...
        const cachedTransaction =
          state.offlineData.cachedData.transactions.find(
  ...
  updateTransactionStatus: async (id, status, notes = "") => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Your transaction will be updated when you are back online.",
        );
      }
      throw error;
    }
      ...
      if (error.message === "OFFLINE_MODE") {
        ...
        if (state.offlineData.cachedData.transactionMessages) {
          const cachedMessages =
            state.offlineData.cachedData.transactionMessages[transactionId] ||
            [];
  ...
  initiatePayment: async (paymentData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot initiate payment while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
  ...
  verifyPayment: async (verificationData) => {
    try {
      ...
      if (error.message === "OFFLINE_MODE") {
        throw new Error(
          "Cannot verify payment while offline. Please try again when you have internet connection.",
        );
      }
      throw error;
    }
        ...
        if (state.offlineData.cachedData.marketPrices) {
          ...
          if (filters.product) {
            filteredPrices = filteredPrices.filter(
              (price) => price.product_id === filters.product,
            );
          }

          if (filters.location) {
            filteredPrices = filteredPrices.filter(
              (price) => price.location.county === filters.location,
            );
          }
  ...
  getPriceTrends: async (filters = {}) => {
    try {
      const response = await api.get("/market-prices/trends", {
        params: filters,
      });
      return response.data;
    } catch (error) {
