...
export function register(config) {
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    ...
    // Handle automatic service worker updates for existing clients
...
// Special handling for network status changes to enhance offline capabilities
export function registerNetworkStatusListener(config) {
  if ('serviceWorker' in navigator && 'onLine' in navigator) {
    ...
    window.addEventListener('online', () => {
      ...
      if ('SyncManager' in window) {
        navigator.serviceWorker.ready
          .then(registration => {
            return registration.sync.register('syncPendingTransactions');
          })
    ...
    if (navigator.onLine) {
      console.log('Application is currently online');
    } else {
